$(document).ready(function() {
  ...
  $("#feedbackSubmit").click(function() {
    ...
    var $dpdate = $('#datepicker').datepicker('getDate');
    if ($dpdate != null) {
       var resseconds = $dpdate.getTime(); 
       if (!contactForm.isValidDate(resseconds)) {
          hasErrors = true;
          contactForm.addError($datepicker);
       }
    }
    else {
       hasErrors = true;
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('#captcha').attr('src', 'library/vender/securimage/securimage_show.php?sid=' + Math.random());
      },
...
var contactForm = {
  ...
  isValidDate: function (resseconds) {
    var $today = new Date();
    var todseconds = $today.getTime();
    var rc = true;
    if (todseconds > resseconds) {
        rc = false;    	
    }
    else 
    if ((resseconds - todseconds) > 31536000000) {
        rc = false;
    }       
    return rc;
  },
